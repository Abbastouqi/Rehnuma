...
async def _resolve_api_key(request: Request) -> tuple[APIKey, User]:
    """Extract and validate API key; return (api_key, user)."""
    ...
    # Reset daily/monthly counters if window has passed
    async with AsyncSessionLocal() as db:
        db_key = await db.get(APIKey, api_key.id)
        day_delta = (now.date() - db_key.day_reset_at.date()).days
        month_delta = (
            (now.year - db_key.month_reset_at.year) * 12
            + (now.month - db_key.month_reset_at.month)
        )
        if day_delta >= 1:
    ...
    # Rate-limit checks (request counts)
    if api_key.requests_today >= limits["req_day"]:
        raise HTTPException(
            status_code=429,
            detail={
                "error": "rate_limit_exceeded",
                "message": f"Daily request limit ({limits['req_day']:,}) exceeded. Resets at midnight UTC.",
                "limit_type": "requests_per_day",
                "limit": limits["req_day"],
    ...
    if api_key.requests_month >= limits["req_month"]:
        raise HTTPException(
            ...
            detail={
                "error": "rate_limit_exceeded",
                "message": f"Monthly request limit ({limits['req_month']:,}) exceeded. Resets at month boundary.",
                "limit_type": "requests_per_month",
                "limit": limits["req_month"],
                "used": api_key.requests_month,
            },
        )

    # Credit check — load the owning user
    async with AsyncSessionLocal() as db:
        user = await db.get(User, api_key.user_id)
        if user is None:
            raise HTTPException(status_code=403, detail="User account not found")
        credits = getattr(user, "credits_balance", 0) or 0
...
async def _log_and_deduct(
    ...
    async with AsyncSessionLocal() as db:
        ...
        if db_key:
            ...
            db_key.tokens_month += total_tokens
            db_key.last_used_at = datetime.now(timezone.utc)
        if db_user and status_code == 200:
            db_user.credits_balance = max(0, (db_user.credits_balance or 0) - total_tokens)
        log = UsageLog(
...
async def chat_completions(req: V1ChatRequest, request: Request):
    api_key, user = await _resolve_api_key(request)
    ...
    try:
        ...
        if req.stream:
            async def event_stream():
                ...
                try:
                    ...
                    import asyncio
                    asyncio.create_task(
                        _log_and_deduct(api_key, user, input_tokens, output_tokens, latency, 200, None, ip)
                    )
        ...
        import asyncio
        asyncio.create_task(
            _log_and_deduct(api_key, user, input_tokens, 0, latency, 500, str(e)[:200], ip)
        )
